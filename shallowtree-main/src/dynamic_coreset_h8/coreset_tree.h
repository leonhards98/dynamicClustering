...
namespace dynClustering
{
	class CoresetTree
	{
		...
		std::vector<std::shared_ptr<CoresetNode>> intermediate_level6;
		int cardOfInternalNodes = 2;
	...
	public:
		CoresetTree(int numberOfClusters, int dimensions, int sizeOfFinalCoreset, int *s, int *numberOfNotCenterCoresetPoints, int *cutOff, int* np, double eps, int clusterFactor, double maxPointsFactor)
		{
			...
			std::shared_ptr<CoresetNode> internalNode6 = std::make_shared<CoresetNode>(numberOfClusters*clusterFactor, s, numberOfNotCenterCoresetPoints, cutOff, np, eps);
			...
			internalNode5->setRoot(internalNode6.get());
			intermediate_level5.push_back(internalNode5);

			internalNode6->children.push_back(internalNode5.get());
			intermediate_level6.push_back(internalNode6);

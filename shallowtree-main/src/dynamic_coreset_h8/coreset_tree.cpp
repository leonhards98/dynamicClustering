...
namespace dynClustering
{
	...
	std::shared_ptr<CoresetNode> CoresetTree::insertLeaf()
	{
		...
		std::shared_ptr<CoresetNode> newInternalNode6;

		if (leafs.size() % (this->cardOfInternalNodes*this->cardOfInternalNodes*this->cardOfInternalNodes*this->cardOfInternalNodes*this->cardOfInternalNodes*this->cardOfInternalNodes) == 0)
		{
			newInternalNode6 = std::make_shared<CoresetNode>(numberOfClusters*clusterFactor,s, numberOfNotCenterCoresetPoints, cutOff, np, eps);
			intermediate_level6.push_back(newInternalNode6);
		}
		else
			newInternalNode6 = this->intermediate_level6.at(this->intermediate_level6.size()-1);
		...
		if (leafs.size() % (this->cardOfInternalNodes*this->cardOfInternalNodes*this->cardOfInternalNodes*this->cardOfInternalNodes*this->cardOfInternalNodes) == 0)
		{
			...
			newInternalNode5->setRoot(newInternalNode6.get());
			newInternalNode6->children.push_back(newInternalNode5.get());
		}
		else
	...
	void CoresetTree::runStaticCoreset(CoresetNode* startLeaf, bool isInit)
	{
		if (isInit || this->numberOfClusters >= startLeaf->data.size())
		{
			...
			startLeaf->root->root->root->root->root->root->clearVectors();
			...
			for (auto &child : startLeaf->root->root->root->root->root->root->children)
			{
				startLeaf->root->root->root->root->root->root->data.insert(startLeaf->root->root->root->root->root->root->data.end(),child->resultData.begin(),child->resultData.end());
				startLeaf->root->root->root->root->root->root->weights.insert(startLeaf->root->root->root->root->root->root->weights.end(),child->resultWeights.begin(),child->resultWeights.end());
			}
			startLeaf->root->root->root->root->root->root->runStaticCoreset();

			root->clearData();
			//!!!Change here also
			for (const auto& internalNode : this->intermediate_level6)
			{

...
using namespace coresetsStatic;
using namespace utilsStatic;

coresetsStatic::SensitivitySampling::SensitivitySampling(size_t numberOfClusters, size_t targetSamplesInCoreset) : TargetSamplesInCoreset(targetSamplesInCoreset),
...
SensitivitySampling::run(const blaze::DynamicMatrix<double> &data, const blaze::DynamicVector<double> &weights)
{
    assert(data.rows() == weights.size());
    clusteringStatic::KMeans kMeansAlg(NumberOfClusters, true, 0);

    auto result = kMeansAlg.run(data, weights, false);
...
std::shared_ptr<Coreset>
SensitivitySampling::generateCoresetPoints(const blaze::DynamicMatrix<double> &data, const clusteringStatic::ClusterAssignmentList &clusterAssignments)
{
...
SensitivitySampling::calcCenterWeights(
    const clusteringStatic::ClusterAssignmentList &clusterAssignments,

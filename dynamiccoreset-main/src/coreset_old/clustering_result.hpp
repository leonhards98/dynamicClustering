...
namespace clusteringStatic
{
    ...
    public:
         ...
         */
        ClusteringResult(const ClusterAssignmentList &clusterAssignments, blaze::DynamicMatrix<double> &centroids, double totalSquaredCost);

        ClusterAssignmentList &getClusterAssignments();

        blaze::DynamicMatrix<double> &getCentroids();

        double getTotalSquaredCost();

    private:
        ClusterAssignmentList clusterAssignments;
        blaze::DynamicMatrix<double> centroids;
        double totalSquaredCost;
    };
}

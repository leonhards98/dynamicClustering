...
void ClusterAssignmentList::assignAll(const blaze::DynamicMatrix<double> &dataPoints, const blaze::DynamicMatrix<double> &centers, const blaze::DynamicVector<double> &weights)
{
...
void ClusterAssignmentList::calcCenters(const blaze::DynamicMatrix<double> &dataPoints, blaze::DynamicMatrix<double> &newCenters)
{
    ...
    for (size_t p = 0; p < n; p++)
    {
        const size_t c = clusteringStatic::getIndex(centerIndices, this->clusters[p]);

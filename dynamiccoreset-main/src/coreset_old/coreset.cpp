...
const blaze::DynamicVector<double>& Coreset::getCenterCoordinates(size_t index)
{
    return (*this->centers.at(index));
    
}

void Coreset::addPoint(size_t pointIndex, double weight)
{
    //Do not require points in the coreset to be unique
...
const std::vector<std::shared_ptr<WeightedPoint>>&
Coreset::getPoints() const
{
    return this->points;
}
